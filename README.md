...
각 게시판은 최근 몇 줄만 보여준다. 코르크 면 오른쪽 위의 **⤢** 버튼을 누르면 전체 목록을 스크롤로 보는 팝업이 뜬다 — 두 게시판 모두 미리보기와 팝업이 같은 데이터를 같은 순서로 보여주므로 잘려서 안 보이던 항목도 여기서 놓치지 않는다. 화면이 좁아지면 게시판은 사무실 아래 카드로 떨어지는데, 이때도 같은 상한만큼만 보이고 ⤢ 버튼은 그대로 남는다.
