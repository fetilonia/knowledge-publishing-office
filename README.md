...
| **Actions: Read and write** | 「초기화」 — 진행 중인 실행 목록 조회와 취소 | 취소만 실패 |
...
Live 모드에서 이 결재는 **PR에 대한 실제 행위**다. 파이프라인은 QA 통과 후 PR을 열고 그 번호를 `status.json`에 실어 보내며, 결재함은 `pr_number`가 있는 job만 올린다. 문서 본문도 그 PR에서 직접 읽는다(`status.json`에는 본문이 담기지 않는다 — 아래 「보안 주의」).
