...
각 게시판은 최근 몇 줄만 보여준다. 코르크 면 오른쪽 위의 **⤢** 버튼을 누르면 전체 목록을 스크롤로 보는 팝업이 뜬다. 진행 기록의 팝업은 미리보기와 같은 한 줄짜리 로그를 그대로 늘린 것이고, 주제 추적의 팝업은 **표**다 — 코르크 미리보기는 `JOB-28`처럼 job_id만 보여 좁은 줄이 지저분해지지 않게 하고, 팝업에서만 Job / 제목(원본 GitHub 이슈 제목) / 상태 / 진행을 별도 칸으로 나눠 보여준다. 화면이 좁아지면 게시판은 사무실 아래 카드로 떨어지는데, 이때도 미리보기는 같은 상한만큼만 보이고 ⤢ 버튼은 그대로 남는다.
...
`status.json`은 job **목록**을 담는다(`schema_version` 0.2.0). 서로 다른 이슈는 병렬로 실행되므로 파일 하나에 여러 job이 동시에 들어 있을 수 있다. 콘솔은 기본적으로 자기가 접수한 job만 따라가고, 다른 job은 「Job 관제」에서 직접 골라야 따라간다.
...
제출한 이슈 번호는 그 탭에서 정해지고 이 탭은 알 수 없으므로, 콘솔은 버튼을 누른 시점에 있던 job 목록을 기억해 두었다가 다음 폴링에서 **처음 나타나는 새 job**을 자동으로 따라간다(`armNewJobAdoption`/`adoptNextNewJob`). 같은 순간 다른 사람이 요청을 넣었다면 잘못 집을 수 있는데, 그럴 때는 「Job 관제」에서 올바른 job으로 옮기면 된다.
...
- **이 Job 따라가기** — 층(사무실)이 그 job을 표시하도록 전환한다. 내가 접수하지 않은 job도 고를 수 있다 — 고르는 행위 자체가 "이걸 보겠다"는 의사표시이므로, 요청한 적 없는 작업이 저절로 움직이지 않게 하려던 제약은 여기에 적용되지 않는다.
...
이건 사무실 애니메이션이 움직이는 범위와는 다르다 — 층은 여전히 내가 접수했거나 「Job 관제」에서 직접 고른 job만 따라간다. 결재함은 그 규칙의 예외다.
