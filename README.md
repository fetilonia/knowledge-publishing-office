...
오른쪽 게시판(**진행 기록**)에는 에이전트별 액션 히스토리가 시간순으로 쌓인다. 그 왼쪽의 두 번째 게시판(**주제 추적**)은 시간순 로그가 아니라 job 단위 현황판이다 — 페이지를 새로 열어도, 내가 접수하지 않은 job이라도, 폴링 중인 `status.json`에 있는 모든 job을 상태와 4단계(연구·작성·교열·QA) 완료 점으로 보여준다. 지금 이 콘솔이 따라가는 job은 이름이 붉게 강조된다. 둘 다 화면이 좁아지면 사무실 아래 카드로 떨어진다.
